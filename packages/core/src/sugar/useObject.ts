...
import { Sugar, SugarGetResult, SugarSetResult, SugarUseObjectResult, SugarValueObject } from "./types";
...
export function useObject<T extends SugarValueObject>(
    ...
    sugar.ready(
      async () => {
        if (!matchSugars(sugar, sugars.current)) {
          console.error("sugarのtemplateとmapのkeyが一致しません。恐らくSugarForm側の問題ですので、報告してください。");
          sugar.destroy();
        ...
        const unavailables = values.filter(([_, value]) => value.result === "unavailable");
        if (unavailables.length > 0) {
          console.error(`Getting useObject sugar: ${unavailables.map(([key, _]) => key).join(", ")} is unavailable.`);
          return { result: "unavailable" };
        }
        ...
        return {
          ...
          value: Object.fromEntries(values.map(([key, value]) => [key, (value as SugarGetResult<unknown> & { result: 'success' }).value])) as T,
        };
      },
      async (value) => {
        if (!matchSugars(sugar, sugars.current)) {
        ...
        // ひとつでも`unavailable`があれば、警告を出して`unavailable`を返す。
        const unavailables = results.filter(([_, value]) => value.result === "unavailable");
        if (unavailables.length > 0) {
          console.error(`Setting useObject sugar: ${unavailables.map(([key, _]) => key).join(", ")} is unavailable.`);
          return { result: "unavailable" };
        }
  ...
  return {
    fields: Object.fromEntries(sugars.current.entries()) as SugarUseObjectResult<T>['fields'],
  };
}
