...
export type SugarUseObject<T extends SugarValue> = T extends SugarValueObject ? (() => SugarUseObjectResult<T>) : never;
...
export type Sugar<T extends SugarValue> = {
  [K in keyof SugarType<T> as SugarType<T>[K] extends never ? never : K]: SugarType<T>[K];
};
